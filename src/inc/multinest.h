...
#else
       #error Dont know how to link to Fortran libraries, check symbol table for your platform (nm libnest3.a | grep nestrun) & edit example_eggbox_C++/eggbox.cc
#endif
...
namespace nested
{
	...
	extern "C" void NESTRUN(int &IS, int &mmodal, int &ceff, int &nlive, double &tol, double &efr, int &ndims, 
	...
	void (*Loglike)(double *Cube, int &n_dim, int &n_par, double &lnew, long &pointer), 
	void (*dumper)(int &, int &, int &, double **, double **, double **, double &, double &, double &, double &, void **), long &pointer, int &root_len);
	...
	void (*LogLike)(double *Cube, int &n_dim, int &n_par, double &lnew, long &pointer), 
	void (*dumper)(int &, int &, int &, double **, double **, double **, double &, double &, double &, double &, void **), void *pointer)
	{
		char t_root[100]; 
        ...
        //NESTRUN( &IS, &mmodal,  &ceff,&nlive,&tol,&efr,&ndims,&nPar,&nClsPar,&maxModes, updInt, Ztol,   root, seed, pWrap,   fb,   resume, &outfile,   initMPI,  &logZero, (*Loglike)(double *Cube, int &n_dim, int &n_par, double &lnew, long &pointer),dumper,&pointer);
		NESTRUN(t_IS,t_mmodal, t_ceff, nlive, tol, efr, 
        ndims, nPar, nClsPar, maxModes, updInt, Ztol, t_root, 
        seed, pWrap, t_fb, t_resume, t_outfile, t_initMPI, 
        logZero, t_maxIter, LogLike, dumper, x, root_len);
	}	
}
